...
window.onload = function(){
    var user = document.querySelector("#userTable");
    var record = "";

    for (var i = 0; i<users.length; i++){
        record += "<tr>"+ "<td>"+
        users[i].first_name + "</td>"+"<td>"+ users[i].last_name + "</td>"+"<td>" + users[i].age + "</td>"+"<td>" +'<a href="mailto: ">' + users[i].email + "</a>" + "</td>" + "</tr>";
    }
    user.innerHTML += record;
};